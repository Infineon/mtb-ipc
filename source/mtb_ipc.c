...
/**
 * \addtogroup group_ipc IPC (Inter-Processor Communication)
 * \{
 */
...
/* DEBUGGING: Enable to check that MTB IPC Semaphore "take" status is the same as the PDL "set"
   state. To enable debugging define _IS_PRIMARY_CORE on main core and _IS_SECONDARY_CORE on other
      core
   and ENABLE_MTB_IPC_DEBUG_PRINT on both.
...
{ \
    ...
    { \
        SCB_InvalidateDCache_by_Addr( (volatile void*)((((uint32_t)(addr) + (size)) & ~(__SCB_DCACHE_LINE_SIZE - 1))), (__SCB_DCACHE_LINE_SIZE)); \
    } \
}
...
#define _MTB_IPC_SERVICE_SEMA_TIMEOUT_US      (2000UL)
...
//  */
#define _MTB_LAST_USER_SEMA_NUM               (_MTB_IPC_SEMA_COUNT - 1)

/* Macro used for determining if we are executing in an ISR at runtime */
#define _MTB_IPC_EXECUTING_IN_ISR()               ((SCB->ICSR & SCB_ICSR_VECTACTIVE_Msk) != 0)
...
#define _MTB_IPC_US_PER_MS                        (1000UL)
#define _IPC_TRIGGER_MASK_UNDEFINED               (0xFFFFUL)
/************************************ FORWARD DECLARATIONS*************************************/
static bool _mtb_ipc_check_if_semaphore_allocated(mtb_ipc_t* obj, uint32_t semaphore_number);
cy_rslt_t _mtb_ipc_semaphore_take_internal(mtb_ipc_semaphore_t* semaphore,
                                           uint64_t* timeout_us);
cy_rslt_t _mtb_ipc_lock_with_timeout(uint32_t channel, uint64_t* timeout_us);
mtb_ipc_queue_event_t _mtb_ipc_queue_all_events_enabled(mtb_ipc_t* obj, uint32_t channel);
bool _mtb_ipc_should_wait_for_irq(uint32_t mask, uint32_t* fired_events);
cy_rslt_t _mtb_ipc_put_get_internal(mtb_ipc_queue_t* queue, void* msg,
                                    mtb_ipc_queue_event_t* triggered_events, bool put);
void _mtb_ipc_mbox_enable_irq(mtb_ipc_mbox_t* mbox, bool enable);
void _mtb_ipc_mbox_signal_other_core(mtb_ipc_mbox_t* mbox);
                                          ...
                                          uint32_t semaphore_num, bool preemptable)
{
    ...
    {
        memset(sema_obj, 0x00, sizeof(mtb_ipc_semaphore_data_t));
...
cy_rslt_t _mtb_ipc_lock_with_timeout(uint32_t channel, uint64_t* timeout_us)
{
    ...
    } while((*timeout_us > 0) && (result != CY_IPC_DRV_SUCCESS));

    if (result == CY_IPC_DRV_SUCCESS)
    {
...
static void _mtb_ipc_initialize_sema_irq(mtb_ipc_t* obj, bool init_core)
{
    ...
    for (uint32_t pool_idx = 0; pool_idx < MTB_IPC_RTOS_SEMA_NUM; ++pool_idx)
    {
        ...
        if (CY_RSLT_SUCCESS == result)
        {
            if (init_core)
            {
                for (int core_num = 0; core_num < _MTB_IPC_CORE_NUM; core_num++)
                {
...
static mtb_ipc_queue_data_t* _mtb_ipc_queue_array_get_pointer(mtb_ipc_t* obj, uint32_t channel_num,
...
static cy_rslt_t _mtb_ipc_queue_array_set_pointer(mtb_ipc_t* obj, uint32_t channel_num,
...
void mtb_ipc_semaphore_process_interrupt(mtb_ipc_t* obj)
{
    ...
    if (semaphore_status_masked != 0)
    {
        #if (defined(CY_RTOS_AWARE) || defined(COMPONENT_RTOS_AWARE)) && (MTB_IPC_RTOS_SEMA_NUM > 0)
        for (uint32_t sema_idx = 0; sema_idx < MTB_IPC_RTOS_SEMA_NUM; sema_idx++)
        {
    ...
    if (mbox_status_masked != 0)
    {
        for (uint8_t mbox_idx = 0u; mbox_idx < MTB_IPC_MAX_MBOXES; mbox_idx++)
        {
            uint32_t mbox_mask = 1u << mbox_idx;
...
void mtb_ipc_queue_process_interrupt(mtb_ipc_t* obj)
{
    ...
    {
        uint32_t channel_mask = CY_IPC_CH_MASK(channel);
        if ((interrupt_status_masked & channel_mask) != 0)
        {
            ...
            while (queue != NULL)
            {
                ...
                if (CY_RSLT_SUCCESS == Cy_IPC_Drv_LockAcquire(ipc_base))
                {
                        ...
                        MTB_IPC_NO_INTR)
                    {
                        flags =
                            ...
                            flags;
                        callback_flags |= (queue->events_enabled & flags);
                ...
                if ((callback != NULL) && (callback_flags != MTB_IPC_NO_INTR))
                {
                                       ...
                                       bool enable)
{
    ...
    if ((event & queue->events_enabled) != 1)
    {
        if (enable)
        {
            // added temporarily for a put/get
            if (((queue->pending_reads == 0) && (event & MTB_IPC_QUEUE_WRITE)) ||
                ((queue->pending_writes == 0) && (event & MTB_IPC_QUEUE_READ)))
            {
                queue->queue_obj->notifications[_MTB_IPC_CUR_CORE_IDX].mask |= event;
            ...
            // this event
            if (event & MTB_IPC_QUEUE_READ)
            {
                queue->pending_writes += 1;
            }
            if (event & MTB_IPC_QUEUE_WRITE)
            {
                queue->pending_reads += 1;
            }
        }
        else
        {
            //decrement counter of entities waiting for this event
            if ((event & MTB_IPC_QUEUE_READ) && (queue->pending_writes > 0))
            {
                queue->pending_writes -= 1;
            }
            if ((event & MTB_IPC_QUEUE_WRITE) && (queue->pending_reads > 0))
            {
                queue->pending_reads -= 1;
            }
...
static cy_rslt_t _mtb_ipc_enable_channel_interrupt(mtb_ipc_t* obj, uint32_t channel,
                                                   bool enable, uint64_t* timeout_us)
{
    ...
    CY_ASSERT(timeout_us != NULL);
    IPC_INTR_STRUCT_Type* ipc_intr_base = Cy_IPC_Drv_GetIntrBaseAddr(obj->queue_irq);
    uint32_t channel_intr_mask = CY_IPC_CH_MASK(channel);
...
mtb_ipc_queue_event_t _mtb_ipc_queue_all_events_enabled(mtb_ipc_t* obj, uint32_t channel)
{
    ...
    while (current_queue != NULL)
    {
        ...
        #endif
        for (uint32_t process_idx = 0; process_idx < _MTB_IPC_MAX_PROCESSES_PER_QUEUE;
             process_idx++)
        {
            INVALIDATE_DCACHE_BEFORE_READING_FROM_MEMORY(current_queue,
                                                         sizeof(mtb_ipc_queue_data_t));
            events_enabled |= current_queue->notifications[process_idx].mask;
        }
...
static uint32_t _mtb_ipc_calculate_trigger_mask(mtb_ipc_t* obj, uint64_t* timeout_us)
{
    ...
    if (obj->trigger_mask == _IPC_TRIGGER_MASK_UNDEFINED)
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            ...
            INVALIDATE_DCACHE_BEFORE_READING_FROM_MEMORY(obj->ipc_shared_vars,
                                                         sizeof(mtb_ipc_shared_t));
            for (int i = 0; i < _MTB_IPC_MAX_PROCESSES_PER_QUEUE; i++)
            {
                                    ...
                                    mtb_ipc_queue_event_t* triggered_events, bool put)
{
    ...
    if (true == put)
    {
        ...
        if (queue->queue_obj->curr_items < queue->queue_obj->max_num_items)
        {
            ...
            uint8_t* queue_item =
                (uint8_t*)((uint32_t)queue->queue_obj->queue_pool +
                           (item_index * queue->queue_obj->item_size));
            memcpy((void*)queue_item, msg, queue->queue_obj->item_size);
            ...
            *triggered_events |= MTB_IPC_QUEUE_WRITE;
            result = CY_RSLT_SUCCESS;
            ...
            if (queue->queue_obj->curr_items == queue->queue_obj->max_num_items)
            {
                *triggered_events |= MTB_IPC_QUEUE_FULL;
            }
            CLEAN_DCACHE_AFTER_WRITING_TO_MEMORY(queue->queue_obj,
                                                 sizeof(mtb_ipc_queue_data_t));
        }
    ...
    else
    {
        ...
        if (queue->queue_obj->curr_items > 0UL)
        {
            ...
            uint32_t next_item = (queue->queue_obj->first_item + 1) %
                                 queue->queue_obj->max_num_items;
            INVALIDATE_DCACHE_BEFORE_READING_FROM_MEMORY(queue_item,
                                                         queue->queue_obj->item_size);
            memcpy(msg, (void*)queue_item, queue->queue_obj->item_size);
            ...
            *triggered_events |= MTB_IPC_QUEUE_READ;
            result = CY_RSLT_SUCCESS;

            /* Trigger interrupt when we empty, but not every time we call if it is already
                empty */
            if (queue->queue_obj->curr_items == 0)
            {
                *triggered_events |= MTB_IPC_QUEUE_EMPTY;
            }
            CLEAN_DCACHE_AFTER_WRITING_TO_MEMORY(queue->queue_obj,
                                                 sizeof(mtb_ipc_queue_data_t));
        }
                                        ...
                                        uint64_t timeout_us, bool put)
{
    ...
    do
    {
        if (wait_irq_enabled)
        {
            wait_for_irq = _mtb_ipc_should_wait_for_irq(event_to_wait, &queue->events_fired);
        }
            ...
            if (result == CY_RSLT_SUCCESS)
            {
                ...
                if (result == CY_RSLT_SUCCESS)
                {
                    ...
                    /** Any modification of a queue's notifs must be done inside a lock on its
                       channel */
                    if (triggered_events != MTB_IPC_NO_INTR)
                    {
                        for (i = 0; i < _MTB_IPC_MAX_PROCESSES_PER_QUEUE; i++)
                        {
                            queue->queue_obj->notifications[i].flags |=
                                queue->queue_obj->notifications[i].mask & triggered_events;
                        }
                        CLEAN_DCACHE_AFTER_WRITING_TO_MEMORY(queue->queue_obj->notifications,
                                                             _MTB_IPC_MAX_PROCESSES_PER_QUEUE *
                                                             sizeof(_mtb_ipc_queue_process_info_t));
                    ...
                    if (!completed && (timeout_us_left > 0))
                    {
                        ...
                        if ((queue->events_enabled & event_to_wait) != true)
                        {
        ...
        if (timeout_us_left > 0)
        {
            timeout_us_left -= 1;
        }
    } while(((timeout_us_left > 0) || never_timeout) && (completed == false));
    ...
    if (modified_mask)
    {
        ...
        uint32_t events_enabled = MTB_IPC_NO_INTR;
        ...
        if (lock_result == CY_RSLT_SUCCESS)
        {
            ...
            events_enabled = _mtb_ipc_queue_all_events_enabled(queue->instance, channel_num);
            /* All channel-related queues have no events enabled, disabling interrupts */
            if (events_enabled == MTB_IPC_NO_INTR)
            {
                (void)_mtb_ipc_enable_channel_interrupt(queue->instance, channel_num,
...
cy_rslt_t mtb_ipc_init(mtb_ipc_t* obj, mtb_ipc_shared_t* shared_data, mtb_ipc_config_t* config)
{
     ...
     * POSC6 devices, the startup code inits the PDL IPC Semaphore list (see system_psoc6_cm0plus.c)
     * On Traveo/XMC7XXX and PSE84 devices, startup code does not initialize IPC PDL semaphore and
     *  does not allocate shared memory for them.
     */
    #if (defined(CY_IP_M7CPUSS) || (defined(CY_IP_M33SYSCPUSS) && \
    defined(CY_IP_M55APPCPUSS))) && \
    !defined(CY_IPC_DEFAULT_CFG_DISABLE)
    ...
    result = (cy_rslt_t)Cy_IPC_Sema_Init(_MTB_IPC_PDL_SEMA_CHAN, _MTB_IPC_SEMA_COUNT,
                                         shared_data->ipc_pdl_sema_array);
    #endif
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        memset(shared_data, 0x00, sizeof(mtb_ipc_shared_t));
...
cy_rslt_t mtb_ipc_get_handle(mtb_ipc_t* obj, mtb_ipc_config_t* config, uint64_t timeout_ms)
{
    ...
    /* Initialize Semaphores for non-boot core.
     * PSOC6 devices, the startup code inits the PDL IPC Semaphore list (see system_psoc6_cm0plus.c)
     * On Traveo/XMC7XXX and PSE84 devices, startup code does not initialize IPC PDL semaphore and
     *  does not allocate shared memory for them.
     */
    /* This code is executing on a NON-INIT (Boot) Core */
    result = (cy_rslt_t)Cy_IPC_Sema_Init(_MTB_IPC_PDL_SEMA_CHAN, 0, NULL);
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        /* Other Core(s) wait for the INIT_CORE to set the Cy_IPC_Drv_ReadDataValue(ipc_base) to be
           non-0x00 */
        uint32_t init_state_shared_vars = 0;
        do
        {
            if (CY_IPC_DRV_SUCCESS == Cy_IPC_Drv_LockAcquire(ipc_base))
            {
        ...
        } while ((init_state_shared_vars == 0) && (timeout > 0UL));
                                           ...
                                           uint64_t* timeout_us)
{
    ...
    if (result != CY_RSLT_SUCCESS)
    {
        ...
        } while (((*timeout_us > 0) || (never_timeout)) &&
...
cy_rslt_t mtb_ipc_semaphore_give(mtb_ipc_semaphore_t* semaphore)
{
    ...
    if (CY_IPC_SEMA_SUCCESS == pdl_ipc_sema_result)
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            ...
            for (int core_num = 0; core_num < _MTB_IPC_CORE_NUM; core_num++)
            {
                for (uint32_t rtos_sema_idx = 0; rtos_sema_idx < MTB_IPC_RTOS_SEMA_NUM;
                     rtos_sema_idx++)
                {
                            ...
                            rtos_sema_idx].sema_idx == semaphore_index)
                    {
                        ...
                        Cy_IPC_Drv_SetInterrupt(ipc_intr_base, 1 << rtos_sema_idx, 0);
                    }
                }
            }
            #else // if (defined(CY_RTOS_AWARE) || defined(COMPONENT_RTOS_AWARE)) &&
                ...
                semaphore_index)
            {
                ...
                Cy_IPC_Drv_SetInterrupt(ipc_intr_base, MTB_IPC_SEMAPHORE_WAIT_MASK, 0);
            }
            #endif \
            /* (defined(CY_RTOS_AWARE) || defined(COMPONENT_RTOS_AWARE)) && (MTB_IPC_RTOS_SEMA_NUM >
               0) */
            /* Release, but do not trigger a release event */
            (void)Cy_IPC_Drv_LockRelease(ipc_base, CY_IPC_NO_NOTIFICATION);
        }
                             ...
                             mtb_ipc_queue_data_t* queue_obj, mtb_ipc_queue_config_t* config)
{
    ...
    if ((config->queue_pool == NULL) ||
        ...
        (config->max_num_items == 0) ||
        (config->item_size == 0) ||
        (config->channel_num == obj->internal_channel_index))
    {
        return MTB_IPC_RSLT_ERR_INVALID_PARAMETER;
    }
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        if (_mtb_ipc_queue_array_get_pointer(obj, config->channel_num, config->queue_num) != NULL)
        {
        ...
        else
        {
            ...
            if (CY_RSLT_SUCCESS == result)
            {
                ...
                queue_obj->curr_items = 0;
                queue_obj->first_item = 0;

                //Local data
                queue->events_fired = MTB_IPC_NO_INTR;
                queue->pending_reads = 0;
                queue->pending_writes = 0;

                for (uint32_t i = 0; i < _MTB_IPC_MAX_PROCESSES_PER_QUEUE; i++)
                {
                    queue_obj->notifications[i].mask = MTB_IPC_NO_INTR;
                    queue_obj->notifications[i].flags = MTB_IPC_NO_INTR;
                }
...
void mtb_ipc_queue_free(mtb_ipc_queue_t* queue)
{
    ...
    if ((CY_RSLT_SUCCESS == result) && (sem_take == CY_RSLT_SUCCESS))
    {
        ...
        if (CY_RSLT_SUCCESS == result)
        {
            ...
            for (i = 0; i < _MTB_IPC_MAX_PROCESSES_PER_QUEUE; i++)
            {
                queue->queue_obj->notifications[i].mask = MTB_IPC_NO_INTR;
                queue->queue_obj->notifications[i].flags = MTB_IPC_NO_INTR;
            }
            IPC_STRUCT_Type* ipc_base =
                                   ...
                                   uint32_t queue_num)
{
    ...
    if ((queue != NULL) && (queue_obj != NULL) && (result == CY_RSLT_SUCCESS))
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            uint32_t timeout = _MTB_IPC_SERVICE_SEMA_TIMEOUT_US;
                                ...
                                bool enable)
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        if (enable == true)
        {
            queue->queue_obj->notifications[process_number].mask |= event;
            queue->events_enabled |= event;
        }
        else
        {
            queue->queue_obj->notifications[process_number].mask &= ~event;
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        if (events_enabled == MTB_IPC_NO_INTR)
        {
            (void)_mtb_ipc_enable_channel_interrupt(queue->instance, channel_num,
                 ...
                 (events_enabled != MTB_IPC_NO_INTR))
        {
            (void)_mtb_ipc_enable_channel_interrupt(queue->instance, channel_num,
                                                    true, &timeout_us);
        }
                                     ...
                                     mtb_ipc_mbox_config_t* config)
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        {
            memset(mbox_obj, 0x00, sizeof(mtb_ipc_mbox_data_t));
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        else
        {
            ...
            if (result != CY_RSLT_SUCCESS)
            {
                ...
                memset(&sender->sema_write_handle, 0x00, sizeof(mtb_ipc_semaphore_t));
                memset(&sender->sema_read_handle, 0x00, sizeof(mtb_ipc_semaphore_t));
            }
            else
            {
                result = CY_RSLT_SUCCESS;
...
void _mtb_ipc_mbox_signal_other_core(mtb_ipc_mbox_t* mbox)
{
    ...
    uint32_t mask = 1 << mbox->mbox_obj->mbox_idx;
...
void _mtb_ipc_mbox_enable_irq(mtb_ipc_mbox_t* mbox, bool enable)
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        uint32_t current_mbox_interrupt_mask = _FLD2VAL(IPC_INTR_STRUCT_INTR_MASKED_NOTIFY, Cy_IPC_Drv_GetInterruptMask(
                                                            ipc_intr_base));
        uint32_t mbox_intr_mask = 1 << mbox->mbox_obj->mbox_idx;
        if (enable)
        {
            //Check that not already enabled
            if ((current_mbox_interrupt_mask & mbox_intr_mask) == 0)
            {
